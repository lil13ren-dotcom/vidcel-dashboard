...
class _HasReviewSnippets(Protocol):
    """Matches both RawCompany (v1) and RawCompanyV2 (Phase 1.1) — this
    module only ever reads review_snippets, so it works unmodified for
    either raw-company representation."""
...
def top_customer_words(company: _HasReviewSnippets, top_n: int = 8) -> list[str]:
    """Most repeated non-stopword tokens across all review snippets."""
    tokens: list[str] = []
    for snippet in company.review_snippets:
        tokens.extend(
            t for t in _tokenize(snippet) if t not in _STOPWORDS and len(t) > 2
        )
    counts = Counter(tokens)
    return [word for word, _ in counts.most_common(top_n)]


def trust_signal_word_hits(company: _HasReviewSnippets) -> dict[str, int]:
    """Which of the fixed TRUST_SIGNAL_WORDS actually appear in reviews,
    and how many times.
    """
    joined = " ".join(company.review_snippets).lower()
    return {word: joined.count(word) for word in TRUST_SIGNAL_WORDS if word in joined}


def has_sufficient_review_text(company: _HasReviewSnippets) -> bool:
