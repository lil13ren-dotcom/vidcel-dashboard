...
def write_companies_csv(scored: list[ScoredCompany], path: str) -> None:
...
def write_company_scores_csv(scored: list[ScoredCompany], path: str) -> None:
    ...
    with open(path, "w", newline="") as f:
        ...
        writer.writeheader()
        for sc in sorted(
            scored, key=lambda s: s.overall_opportunity_score, reverse=True
        ):
            writer.writerow(sc.as_flat_dict())


def write_opportunities_csv(scored: list[ScoredCompany], path: str) -> None:
