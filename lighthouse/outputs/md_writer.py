...
def _present_absent(company: RawCompany) -> tuple[list[str], list[str], list[str]]:
    present = [
        SIGNAL_LABELS[k]
        for k in WEBSITE_SIGNAL_KEYS
        if getattr(company.website_signals, k)
    ]
    absent = [
        SIGNAL_LABELS[k]
        for k in WEBSITE_SIGNAL_KEYS
    ...
    unverified = [
        SIGNAL_LABELS[k]
        for k in RENDER_ONLY_SIGNAL_KEYS
...
def write_company_report(sc: ScoredCompany, path: str) -> None:
    ...
    lines.append(
        f"| **Overall Opportunity Score** | **{round(sc.overall_opportunity_score, 1)}** |"
    )
    lines.append("")
    ...
    if sc.top_customer_words:
        ...
        lines.append("")
        lines.append(
            f"Most repeated words in public reviews: {', '.join(sc.top_customer_words)}"
        )
        lines.append("")
    ...
    if top_opp:
        lines.append(
            f"**{top_opp.title}** (Impact: {top_opp.expected_impact}, "
...
def write_industry_summary(scored: list[ScoredCompany], path: str) -> None:
    ...
    for industry in industries:
        ...
        def avg(attr: str, group: list[ScoredCompany] = group) -> float:
            return round(float(stats.mean(getattr(sc, attr) for sc in group)), 1)
        ...
        for key in WEBSITE_SIGNAL_KEYS:
            if key in RENDER_ONLY_SIGNAL_KEYS:
                continue
            missing = sum(
                1 for sc in group if not getattr(sc.company.website_signals, key)
            )
            if missing:
        ...
        if gap_counts:
            ...
            for key, count in sorted(
                ...
                lines.append(
                    f"- {SIGNAL_LABELS[key]}: missing in {count}/{len(group)} companies ({pct}%)"
                )
            lines.append("")
        ...
        ranked = sorted(
            group, key=lambda sc: sc.overall_opportunity_score, reverse=True
        )
    ...
    by_industry_avg = sorted(
        industries,
        key=lambda ind: stats.mean(
            sc.overall_opportunity_score for sc in scored if sc.company.industry == ind
        ),
        reverse=True,
    )
    ...
    top_avg = round(
        stats.mean(
            sc.overall_opportunity_score
            for sc in scored
            if sc.company.industry == top_industry
        ),
        1,
    )
    lines.append(
        f"**{top_industry}** shows the highest average Overall Opportunity Score in this POC sample "
